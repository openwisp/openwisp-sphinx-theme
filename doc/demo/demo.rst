...
citation references ([12]_), substitution references, and _`inline hyperlink targets`
(see Targets_ below for a reference back to here). Character-level inline markup is also possible
(although exceedingly ugly!) in *re*\ ``Structured``\ *Text*. Problems are indicated by lorem.
...
.. code-block:: console
    ...
    $ curl https://etmsbxsssw.org/flu/seasonal/h3n2/ha/2y                       \
        --header 'Accept: application/vnd.etmsbxsssw.dataset.main+json'         \
        --compressed                                                            \
